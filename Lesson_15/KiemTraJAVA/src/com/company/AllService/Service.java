package com.company.AllService;
    ...
    public void forgotPass() {
        ...
        for (int i = 0; i < listUser.size(); i++) {
            if (listUser.get(i).getEmail().equals(email)) {
                System.out.println("Nhập Email chính xác, tiên hành nhập mật khẩu mới: ");
                String newPassword = CheckInput.checkPassword();
                listUser.get(i).setPassword(newPassword);
                goLogin();
                count++;
        ...
        if (count == 0) {
            System.out.println("Không tồn tại email này, vui lòng kiểm tra lại");
            forgotPass();
        }
    }
